		...
		>
			...
			<div className="relative grid grid-cols-2 gap-2 lg:gap-7 pl-[2vw] px-[2.5vw] my-5 ">
				...
				{data?.slice(0, 2)?.map((item, index) => (
                    <Link key={index} href={item.slug} className="group text-center flex flex-col justify-between sm:even:flex-col-reverse even:mt-16 odd:mb-16 sm:last:hidden lg:last:flex border sm:border-0 border-gray-300 overflow-hidden sm:pb-0"> 
                        <Image
                            src={item.image ?? "/assets/placeholder/collection.svg"}
