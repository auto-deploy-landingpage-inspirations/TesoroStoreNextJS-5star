	...
	return (
		<div className="flex py-4 items-center lg:px-3 border-b border-gray-300">
			<div className="flex border rounded-md border-gray-300 w-16 h-16 flex-shrink-0">
				<img
					src={item.image && item.image.length > 0 ? item?.image : "/assets/placeholder/order-product.svg"}
					width="64"
					height="64"
			...
			<h6 className="text-sm ps-3 font-regular text-heading">
				{generateCartItemName(item.name, item.attributes)} <span style={{fontSize: '12px'}} className="text-muted"> x {item.quantity} units</span>
			</h6>
